        ...
        jsonReader("./json/polls.json", (err, data) => {
          if (err) {
            ...
            if (canRespond === true && userHasRole === true) {
              ...
              .setAuthor(data[placeOnArray].poll[7].embed.author.name, data[placeOnArray].poll[7].embed.author.icon_url)
              let totalVotes = 0; 
              let iteration5 = 0;
              for (answers in data[placeOnArray].poll[4].possibleAnswers) {
                totalVotes = totalVotes + data[placeOnArray].poll[4].possibleAnswers[iteration5].answerContent[1].votes;
                iteration5++;
              }
              totalVotes++;
              embed.setFooter(`please select an option from the dropdown below\ntotal responses: ${totalVotes}`);
            ...
            jsonReader("./json/polls.json", (err, data0) => {
              if (err) {
                ...
                const ended = new MessageEmbed()
                  ...
                  .setThumbnail(data0[pollPlaceOnArray].poll[7].embed.thumbnail.url)
                  .setAuthor(data0[pollPlaceOnArray].poll[7].embed.author.name, data0[pollPlaceOnArray].poll[7].embed.author.icon_url);

                  const channelToUpdate = interaction.guild.channels.cache.find(c => c.id === data0[pollPlaceOnArray].poll[8].channelId)
                  const messageToUpdate = channelToUpdate.messages.fetch(data0[pollPlaceOnArray].poll[9].messageId).then(m => m.edit({ embeds: [ended], components: [] }))
