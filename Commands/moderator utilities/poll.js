...
module.exports = {
  ...
  async execute(interaction) {
    ...
    if (interaction.options.getSubcommand() === "create") {
      ...
      const canInitPoll =
        interaction.member.roles.cache.some((r) => r.name === "Moderators") ||
        interaction.member.roles.cache.some((r) => r.name === "Survey Creator");
      ...
      options = testOptions;
      if (options.length <= 1 ) return interaction.followUp({ embeds: [minans] });
      ...
      let message = await pollChannelSend.send({ embeds: [embed], components: [row] });
