...
const surveyChannel = require('../../settings.json');
let channelPostGlobal;
let surveyTitleGlobal;
...
module.exports = {
    ...
    async execute(interaction) {
        const canInitSrv = interaction.member.roles.cache.some(r => r.name === 'Moderators') || interaction.member.roles.cache.some(r => r.name === 'Survey Creator');
        if (interaction.options.getSubcommand() === 'create') {
            ...
            if (canInitSrv) {
                ...
                const pollConf = new MessageEmbed()
                    .setColor("RANDOM")
                    .setDescription("Your survey has been created");
                ...
                channelPostGlobal = channelPost;
                surveyActive = true;
                console.log(`survey active: ${surveyActive}`);
                interaction.followUp({ embeds: [pollConf] });

                const threadName = `${surveytitle}`;
                const threadChannel = interaction.guild.channels.cache.find(c => c.id === surveyChannel);
                ...
                if (surveyActive == true) {
                    if (canInitSrv) {
                        const conf = new MessageEmbed()
                            ...
                            const channel = interaction.guild.channels.cache.find(c => c.id === surveyChannel);
                            const thread = channel.threads.cache.find(x => x.name === threadNameGlobal);
                            await thread.setArchived(true);

                        const surveyEnd = new MessageEmbed()
                        .setColor('RANDOM')
                        .setTitle('The survey has ended')
                        .setDescription(`The survey: "${surveyTitleGlobal}" has ended.`);

                        channelPostGlobal.send({ embeds: [surveyEnd] });
                        surveyActive = false;
